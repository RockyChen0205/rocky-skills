...
### Q: 为什么 Jina Reader 返回 "Page not found"？

A: 常见原因：
1. **URL 构造错误**（最常见）：拼接 Jina URL 时不要拆分/截断原始 URL 中的 ID
   - ❌ 错误：`https://r.jina.ai/https://x.com/user/status1234567890123456789` → 错误拆分 ID
   - ✅ 正确：`https://r.jina.ai/https://x.com/user/status/1234567890123456789`
   - **注意**：Twitter/X 的 tweet ID 是 18-19 位数字，复制时务必保持完整
2. 页面已删除或不可见
3. 账号被冻结
