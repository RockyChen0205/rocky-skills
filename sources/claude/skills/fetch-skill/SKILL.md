...
`~/.claude/skills/fetch-skill/fetch.py`

## 使用方法

```bash
# 抓取任意网页
python3 ~/.claude/skills/fetch-skill/fetch.py https://example.com

# 保存到文件
python3 ~/.claude/skills/fetch-skill/fetch.py https://example.com -o output.md

# 静默抓取
python3 ~/.claude/skills/fetch-skill/fetch.py https://example.com -q

# 纯文本输出
python3 ~/.claude/skills/fetch-skill/fetch.py https://example.com -t
```
...
```bash
# 单条推文（无需登录）
python3 ~/.claude/skills/fetch-skill/fetch.py https://x.com/OpenAI/status/123456 -t

# 推文 JSON
python3 ~/.claude/skills/fetch-skill/fetch.py https://x.com/OpenAI/status/123456 --pretty

# 推文 + 回复（需要 Camofox）
python3 ~/.claude/skills/fetch-skill/fetch.py https://x.com/OpenAI/status/123456 --replies -t
```
...
```bash
# 默认（Jina 兜底）
python3 ~/.claude/skills/fetch-skill/fetch.py "https://mp.weixin.qq.com/s/xxxx"

# 使用 wechat-article-exporter
python3 ~/.claude/skills/fetch-skill/fetch.py "https://mp.weixin.qq.com/s/xxxx" --wechat-api http://localhost:3000
```
